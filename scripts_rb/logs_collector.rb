...
def insert_data(client)
  begin
    # Lecture du fichier de logs Apache (access.log)
    apache_log_path = '/var/log/apache2/access.log' 
    File.foreach(apache_log_path) do |line|
      # Vérification du format des lignes de log
    ...
    puts "Erreur lors de l'insertion des logs Apache : #{e.message}"
  end

  begin
    # Lecture du fichier de logs MariaDB (error.log)
    mariadb_log_path = '/var/log/mysql/error.log'  # Assurez-vous que ce chemin est correct dans votre conteneur
    ...
    puts "Erreur lors de l'insertion des logs MariaDB : #{e.message}"
  end 
end
